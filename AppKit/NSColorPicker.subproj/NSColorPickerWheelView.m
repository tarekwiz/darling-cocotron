...
- (void)_updateHandleLocation
{
	...
	CGFloat colorWheelRadius = .5 * MIN(NSWidth(bounds), NSHeight(bounds));
	
	CGFloat angle = M_PI * _hueValue/ 180.f;
...
- (void)mouseDragged:(NSEvent*)event
{
	...
	CGFloat relativeDistanceFromCenter = hypot(location.x, location.y)/colorWheelRadius;
	if (relativeDistanceFromCenter > 1)
		relativeDistanceFromCenter = 1;
	
	// Get the angle and ensure it's in 0..2*PI
	CGFloat angle = fmod(atan2(location.y, location.x) - M_PI_2 + M_PI * 2.f, M_PI * 2.f);
